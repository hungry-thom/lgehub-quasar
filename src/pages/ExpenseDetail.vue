  ...
  <q-page class="layout-padding">
    <div>
      ...
      <div class="row no-wrap">
        <q-input class= "col" :readonly="noEdit" minimal color="orange" v-model="transaction.date1.substr(0,10)" float-label="Date"/>&nbsp;&nbsp;
        <q-input class= "col" ref="inputVendor" :readonly="noEdit" v-model="transaction.vendor" float-label="Vendor"  @blur="validateVendor"> <q-autocomplete :static-data="{field: 'value', list: vendorsList}" /></q-input>&nbsp;&nbsp;
        <q-input class= "col" ref="inputtransNum" :readonly="noEdit" v-model="transaction.transNum" float-label="Transaction Number"/>&nbsp;&nbsp;
        <q-input class= "col" v-model="transaction.paymentAccount" :readonly="noEdit" float-label="Payment Account" > <q-autocomplete :static-data="{field: 'value', list: paymentTypes}" /></q-input>
      </div>
      <div>
        <br>
        <q-table
          ...
          <q-tr slot="body" slot-scope="props" :props="props">
            ...
            <q-td key="expand" :props="props">
              <div class="row items-center justify-between no-wrap">
                <q-checkbox v-model="props.row.add2Pricelist" :readonly="noEdit" checked-icon="attach_money" unchecked-icon="money_off" />
                <q-checkbox v-model="props.row.add2Inventory" :readonly="noEdit" checked-icon="assignment" unchecked-icon="no_sim" class="q-mr-sm" />
              </div>
            </q-td>
          </q-tr>
          <!--
    ...
    <div class="q-pa-sm">
    <div class="row no-wrap">
      <q-datetime class= "col" minimal color="orange" v-model="editTransaction.date1" type="date" float-label="Date" @input="selectDate" :first-day-of-week="0" />&nbsp;&nbsp;
      <q-input class= "col" ref="inputVendor" v-model="editTransaction.vendor" float-label="Vendor"  @blur="validateVendor"> <q-autocomplete :static-data="{field: 'value', list: vendorsList}" /></q-input>&nbsp;&nbsp;
      <q-input class= "col" ref="inputtransNum" v-model="editTransaction.transNum" float-label="Transaction Number"/>&nbsp;&nbsp;
      <q-input class= "col" v-model="editTransaction.paymentAccount" float-label="Payment Account" > <q-autocomplete :static-data="{field: 'value', list: paymentTypes}" /></q-input>
    </div>
    <div>
      <br>
      <q-table
        :data="editTransaction.transItems"
        :columns="modalColumns"
        :visible-columns="visibleModalColumns"
        row-key="item"
        :pagination.sync="pagination"
        ...
        <q-tr slot="bottom-row" slot-scope="props" align="left">
            ...
            <q-td></q-td>
            <q-td></q-td>
            <q-td class="bg-deep-purple-2">{{ editSubTotal || '-' }}</q-td>
            <q-td class="bg-deep-purple-2">{{ editGstTotal || '-' }}</q-td>
            <q-td class="bg-deep-purple-3">{{ editGrandTotal || '-' }}</q-td>
            <q-td></q-td>
            <q-td></q-td>
        </q-tr>
      </q-table>
...
export default {
  ...
  computed: {
    ...
    editSubTotal () {
      let t = 0
      this.$data.editTransaction.transItems.forEach(item => {
        console.log('t', t, item.cost)
        t += item.cost
      })
      return _.round(t,2)
  ...
  methods: {
    ...
    popup(row) {
      ...
      console.log(row)
      this.$data.editTransaction = JSON.parse(JSON.stringify(this.$data.transaction))
      this.overlay()
      /*
      this.$q.notify({
        message: row.id,
    ...
    loadData () {
      ...
      api.service('expenses').find({
        query: {
          $sort: { item: 1 },
          $limit: 500, // maximum is 200
          id: this.$route.params.expId
        }
      }).then((response) => {
        console.log('resp', response)
        this.$data.transaction = response.data[0]
        // this.overlay()
      })

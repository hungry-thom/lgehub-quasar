...
export default {
  ...
  methods: {
    showNotification (val) {
      ...
      this.$data.categoryItems = this.$data.categories[val]
    },
    loadCategories () {
    ...
    loadTransferData(lookupDate) {
      ...
      api.service('transfers').find({
        ...
        if (response.data.length > 0) {
          ...
          // need to iterate each item and categorize
          let categories = this.$data.categories 
          response.data[0].itemList.forEach(item => {
            // add item's category to categories if not already present
            if (!categories[item.category]) {
              categories[item.category] = []
              this.$data.categoryArray.push(item.category)
            }
          ...
          // load new starting date
          let d = new Date(response.data[0].week)
          ...
          // check for previous and next week
          this.checkPrevNextButtons()
          this.$data.categoryItems = categories['DryFood']
